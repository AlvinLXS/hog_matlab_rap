function [resultRects,totalWindows] = searchImage(hog, origImg)
...
%                  (the classifier confidence).
	...
	scaleRange = zeros(3);
 	scale = 1.0;
% 	for i = 1 : 1
% 		scaleRange(i) = scale/2;
% 		%scale = scale / 1.05;
% 	end
    scaleRange(1) = scale/2;
    scaleRange(2) = scale/3;
    scaleRange(3) = scale/4;
	...
	for i = 1 : length(scaleRange)
		...
		if (scale == 1)
			...
			img = imresize(origImg, scale);%image zoom in/out
        end
		...
		while((cellRow + hog.numVertCells - 1) <= size(allHistograms, 1))
			...
			cellRowRange = cellRow : (cellRow + hog.numVertCells - 1);	%hog.numVertCells=numofblock	
			...
			while ((cellCol + hog.numHorizCells - 1) <= size(allHistograms, 2))
				...
				if (p > hog.threshold)
					...
					resultRects = [resultRects; origX, origY, origWidth, origHeight, p];
				end               
